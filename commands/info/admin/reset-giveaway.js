...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        schema.findOne({ Guild: message.guild.id }, async(err, data) => {
            if(err) throw err
            if(!data) {
                message.channel.send(`<:wrong:893330231954386984> Giveaway role for this server is reseted already`)
            } else {
                await schema.findOneAndDelete({ Guild: message.guild.id })
                message.channel.send(`<:right:893330260295311360>  Giveaway role has been disabled`)
            }
        })
    } catch (error) {
        console.log(error)
